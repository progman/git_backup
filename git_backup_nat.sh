...
function main()
{
	echo "$(get_time)  run git_backup_nat v0.0.1 (https://github.com/progman/git_backup.git)";
	echo "$(get_time)  use backup dir \"${GIT_BACKUP_NAT_CACHE_DIR}\"";
...
# create cache dir
	if [ ! -d "${GIT_BACKUP_NAT_CACHE_DIR}" ];
	then
		mkdir -- "${GIT_BACKUP_NAT_CACHE_DIR}" &> /dev/null < /dev/null;
		if [ "${?}" != "0" ];
		then
			echo "$(get_time)! ERROR: dir GIT_BACKUP_NAT_CACHE_DIR not found";
			echo;
			echo;
			return 1;
		fi
...
# get list
	if [ ! -f "${GIT_BACKUP_NAT_LIST}" ];
	then
		echo "$(get_time)! ERROR: file GIT_BACKUP_NAT_LIST not found";
		echo;
		echo;
		return 1;
	fi
